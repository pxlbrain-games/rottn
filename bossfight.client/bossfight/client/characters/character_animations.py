...
class AnimatedCharacter(cocos.batch.BatchableNode):
    def __init__(self, moving_parent, body_spritesheet='clothes.png', head_spritesheet='male_head1.png', \
      weapon_spritesheet=None, shield_spritesheet=None, scale=2.5):
        ...
        body_spritesheet = pyglet.image.ImageGrid(pyglet.resource.image(body_spritesheet), 8, 32)
        head_spritesheet = pyglet.image.ImageGrid(pyglet.resource.image(head_spritesheet), 8, 32)
        idle_sprites = {
            CharacterPart.Body: create_animated_sprites(body_spritesheet, 0, 3, 0.2, scale),
            CharacterPart.Head: create_animated_sprites(head_spritesheet, 0, 3, 0.2, scale)
        }
        running_sprites = {
            CharacterPart.Body: create_animated_sprites(body_spritesheet, 4, 11, 0.09, scale),
            CharacterPart.Head: create_animated_sprites(head_spritesheet, 4, 11, 0.09, scale)
        }
        attacking_sprites = {
            CharacterPart.Body: create_animated_sprites(body_spritesheet, 12, 15, 0.12, scale, False),
            CharacterPart.Head: create_animated_sprites(head_spritesheet, 12, 15, 0.12, scale, False)
        }
        if weapon_spritesheet is not None:
            weapon_spritesheet = pyglet.image.ImageGrid(pyglet.resource.image(weapon_spritesheet), 8, 32)
            ...
            attacking_sprites[CharacterPart.Weapon] = create_animated_sprites(weapon_spritesheet, 12, 15, 0.12, scale, False)
        if shield_spritesheet is not None:
            shield_spritesheet = pyglet.image.ImageGrid(pyglet.resource.image(shield_spritesheet), 8, 32)
            ...
            attacking_sprites[CharacterPart.Shield] = create_animated_sprites(shield_spritesheet, 12, 15, 0.12, scale, False)
        ...
        for character_part in self.sprites[AnimationState.Attacking]:
            for sprite in self.sprites[AnimationState.Attacking][character_part].values():
                self.add(sprite)
                sprite.visible = False
    ...
    def _update_animation_state(self, dt):
        v_squared = euclid.Vector2(self.moving_parent.velocity[0], self.moving_parent.velocity[1]).magnitude_squared()
    ...
    def trigger_animation(self, animation_state):
        for character_part in self.sprites[self.animation_state]:
            self.sprites[animation_state][character_part][self.direction_state].restart_animation()
            self.sprites[self.animation_state][character_part][self.direction_state].visible = False
            self.sprites[animation_state][character_part][self.direction_state].visible = True
        self.animation_state = animation_state
        frames = self.sprites[animation_state][CharacterPart.Body][self.direction_state]._animation.frames
        duration = sum([frame.duration if frame.duration is not None else frames[0].duration for frame in frames])
...
def create_animated_sprites(spritesheet, start, end, duration=0.1, scale=1, loop=True):
    animations = [create_animation(spritesheet, i, start, end, duration, loop) for i in range(0, 8)]
    sprites = {i: AnimatedSprite(image=animations[i-1], position=(0, 75), scale=scale) for i in range(1, 9)}
    return sprites

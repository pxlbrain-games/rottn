...
class ControllableNode(cocos.cocosnode.CocosNode, pyglet.event.EventDispatcher):
    ...
    def _update_movement(self, dt):
        new_joy_direction = (
            euclid.Vector2(self.joystick.rx, -self.joystick.ry)
            if self.joystick is not None
            else None
        )
        if (
            new_joy_direction is not None
        ...
        if (
            self.joystick is None
            or self.joystick.x * self.joystick.x + self.joystick.y * self.joystick.y
            < 0.05
